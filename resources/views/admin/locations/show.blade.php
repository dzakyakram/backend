...
<a href="{{ route('admin.locations.index') }}" style="display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;letter-spacing:1px;color:var(--black);text-decoration:none;margin-bottom:20px;">← SEMUA LOKASI</a>
...
<div class="detail-grid" style="display:grid;grid-template-columns:1fr 320px;gap:20px;">
  ...
  <div style="display:grid;gap:20px;">
    ...
    <div class="brut-card-lg" style="overflow:hidden;">
      <div style="height:280px;background:#e8e4d8;position:relative;border-bottom:3px solid var(--black);">
        ...
        @else
          <div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;">
            <span style="font-size:72px;">{{ $location->kategori === 'wisata' ? '🏔️' : ($location->kategori === 'kuliner' ? '🍜' : '🏨') }}</span>
            <span style="font-size:11px;font-weight:700;color:#888;letter-spacing:2px;">TIDAK ADA FOTO</span>
          </div>
        @endif
      ...
      <div style="padding:22px 24px;">
        <h2 style="font-family:'Syne',sans-serif;font-size:24px;font-weight:800;letter-spacing:-0.5px;margin-bottom:6px;">{{ strtoupper($location->nama) }}</h2>
        <div style="font-size:12px;color:#666;margin-bottom:12px;">&#9679; {{ $location->alamat ?? '—' }}</div>
    ...
    <div class="brut-card" style="overflow:hidden;">
      ...
      <div style="padding:10px 16px;border-top:3px solid var(--black);background:#e8e4d8;display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
        ...
        <a href="https://www.google.com/maps?q={{ $location->latitude }},{{ $location->longitude }}"
           target="_blank" class="btn btn-ghost btn-sm">GOOGLE MAPS ↗</a>
      </div>
    </div>
  </div>
  ...
  <div style="display:grid;gap:16px;align-content:start;">
    ...
    <div style="border:3px solid var(--black);box-shadow:var(--shadow-lg);overflow:hidden;">
      ...
      <div style="padding:16px;display:grid;gap:8px;">
        ...
        <button onclick="document.getElementById('rp').style.display=document.getElementById('rp').style.display==='none'?'block':'none'"
                class="btn btn-red" style="width:100%;justify-content:center;font-size:12px;padding:12px;">✗ TOLAK LOKASI INI</button>
        <div id="rp" style="display:none;border:3px solid var(--red);padding:12px;background:#fff5f5;">
          <form method="POST" action="{{ route('admin.locations.reject', $location) }}">
            ...
            <textarea name="catatan" rows="3" placeholder="ALASAN PENOLAKAN (OPSIONAL)..."
            ...
            <button type="submit" class="btn btn-red" style="width:100%;justify-content:center;margin-top:8px;">KONFIRMASI TOLAK</button>
          </form>
        </div>
      </div>
    </div>
      ...
      <div style="padding:16px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
          ...
          <div>
            ...
            <span class="badge badge-{{ $location->user->role ?? 'pengguna' }}" style="margin-top:4px;display:inline-block;">{{ strtoupper($location->user->role ?? '–') }}</span>
          </div>
        </div>
        <div style="display:grid;gap:6px;border-top:2px solid var(--black);padding-top:12px;font-size:11px;">
          ...
          <div style="display:flex;justify-content:space-between;">
            <span style="color:#888;">TERAKHIR DIUBAH</span>
